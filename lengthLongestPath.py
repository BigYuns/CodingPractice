...
class Solution(object):
    def lengthLongestPath(self, str):
        ...
        for i in range(0,len(arr)):
            original = arr[i]
            arr[i] = arr[i].replace('\t','')
            height = len(original) - len(arr[i])

            if height ==0: 
                path[height]=len(arr[i])
            else: 
                path[height]=len(arr[i])+1+path[height-1]

            if '.' in arr[i]:
                max = path[height] if max < path[height] else max
